...
entry:
	...
	mov ax, 0x0820	;!!!本次添加部分 指定ES=0x0820, BX=0，所以软盘的数据将被装载到内存中0x8200到0x83ff的地方。0x8000～0x81ff这512字节是留给启动区的，要将启动区的内容读到那里。那为什么使用0x8000以后的内存呢？0x7c00～0x7dff用于启动区，0x7e00以后直到0x9fbff为止的区域都没有特别的用途，操作系统可以随便使用
	mov es, ax		;!!!本次添加部分
	mov ch, 0		;!!!本次添加部分 柱面0
	mov dh, 0		;!!!本次添加部分 磁头0
	mov cl, 2		;!!!本次添加部分 扇区2
